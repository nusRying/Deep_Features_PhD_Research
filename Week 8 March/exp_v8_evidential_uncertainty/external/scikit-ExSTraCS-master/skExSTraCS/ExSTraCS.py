...
class ExSTraCS(BaseEstimator,ClassifierMixin):
    ...
    def predict_evidential(self, X, K=1.0):
        """
        Novel EUQ-LCS feature: Test Evidential Uncertainty of ExSTraCS
        Returns the Dempster-Shafer Belief Masses for each class, plus the ignorance (Theta) mass.
        Parameters
        X: array-like {n_samples, n_features} Test instances to classify.
        K: float. The base prior ignorance constant.

        Returns
        y: array-like {n_samples, n_classes + 1} Evidential Belief Masses [prob_0, prob_1, ..., ignorance]
        """
        try:
            for instance in X:
                for value in instance:
        ...
        for inst in range(instances):
            ...
            self.population.makeEvalMatchSet(self,state)
            prediction = Prediction(self, self.population)
            masses = prediction.getEvidentialProbabilities(K=K)
            evidential_list.append(masses)
            self.population.clearSets()
            
        return np.array(evidential_list)
