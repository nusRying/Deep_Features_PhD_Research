...
logger.info(f"DEBUG: Checking for data at: {HYBRID_ISIC_FILE}")
if not HYBRID_ISIC_FILE.exists():
    logger.error(f"CRITICAL: Data file not found at {HYBRID_ISIC_FILE}")
...
def extract_rules_from_ensemble(models_tier1, feature_names):
    ...
    for idx, model in enumerate(models_tier1):
        try:
            # model is an ExSTraCS instance. Correct access: model.population.popSet
            population = model.population.popSet
            for cl in population:
                # Manual Rule Formatting (as seen in IterationRecord.py)
                rule_parts = []
                for i, attr_idx in enumerate(cl.specifiedAttList):
                    attr_name = feature_names[attr_idx]
                    cond = cl.condition[i]
                    if isinstance(cond, list): # Continuous [min, max]
                        rule_parts.append(f"{attr_name} in [{cond[0]:.4f}, {cond[1]:.4f}]")
                    else: # Discrete
                        rule_parts.append(f"{attr_name} == {cond}")
                
                rule_str = " AND ".join(rule_parts) if rule_parts else "DEFAULT"
                ...
                master_rules.append({
                    ...
                    'prediction': cl.phenotype # In Classifier.py, it's .phenotype
                })
