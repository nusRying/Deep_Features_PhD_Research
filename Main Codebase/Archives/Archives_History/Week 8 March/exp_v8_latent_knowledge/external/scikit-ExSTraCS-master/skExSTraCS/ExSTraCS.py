...
class ExSTraCS(BaseEstimator,ClassifierMixin):
    ...
    def saveFinalMetrics(self):
        self.finalMetrics = [self.learning_iterations,self.timer.globalTime, self.timer.globalMatching,self.timer.globalCovering,
                             ...
                             copy.deepcopy(self.env),copy.deepcopy(self.population.latent_archive),copy.deepcopy(self.population.popSet),self.preRCPop]
    ...
    def rebootPopulation(self):
        ...
        # Recover Latent Archive if it exists in the pickle (is v8 LKH-LCS version)
        if len(rawData) >= 20:
            set.latent_archive = rawData[len(rawData)-2]
    ...
    def predict(self, X):
        ...
        for inst in range(instances):
            ...
            if getattr(prediction, 'rescued_by_latent', False):
                self.rescued_predictions += 1
