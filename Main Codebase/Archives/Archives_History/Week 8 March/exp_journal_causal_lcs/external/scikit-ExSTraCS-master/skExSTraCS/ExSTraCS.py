...
from skExSTraCS.ExtensionHooks import DefaultFitnessPolicy, DefaultParentSelectionPolicy, DefaultSubsumptionPolicy, clone_initial_population
import copy
import time
import pickle
import random
...
class ExSTraCS(BaseEstimator,ClassifierMixin):
    def __init__(self,learning_iterations=100000,N=1000,nu=1,chi=0.8,mu=0.04,theta_GA=25,theta_del=20,theta_sub=20,
                 ...
                 track_accuracy_while_fit=False,random_state=None,fitness_policy=None,parent_selection_policy=None,
                 subsumption_policy=None,initial_population=None):
        '''
        ...
        self.fitness_policy = fitness_policy if fitness_policy is not None else DefaultFitnessPolicy()
        self.parent_selection_policy = parent_selection_policy if parent_selection_policy is not None else DefaultParentSelectionPolicy()
        self.subsumption_policy = subsumption_policy if subsumption_policy is not None else DefaultSubsumptionPolicy()
        self.initial_population = clone_initial_population(initial_population)
        ...
        if self.reboot_filename != None:
            ...
            self.population = self._make_empty_population()

    def _make_empty_population(self):
        population = ClassifierSet()
        if self.initial_population is not None:
            population.popSet = clone_initial_population(self.initial_population)
            population.microPopSize = sum(cl.numerosity for cl in population.popSet)
        return population

    def get_fitness(self, classifier):
        return self.fitness_policy.calculate(self, classifier)

    def select_parents(self, population):
        return self.parent_selection_policy.select(self, population)

    def is_more_general(self, candidate, other):
        return self.subsumption_policy.is_more_general(self, candidate, other)

    def can_subsume(self, parent, child):
        return self.subsumption_policy.can_subsume(self, parent, child)
