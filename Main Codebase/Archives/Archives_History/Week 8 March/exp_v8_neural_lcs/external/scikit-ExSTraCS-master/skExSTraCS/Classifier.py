...
class Classifier:
    def __init__(self,model):
        ...
        # MN-LCS Core: Micro Neural Network
        # Small hidden layer, warm start allows iterative partial_fit by the GA
        self.micro_mlp = MLPClassifier(hidden_layer_sizes=(8,), max_iter=1, warm_start=True, learning_rate_init=0.01, random_state=random.randint(0,100000))
    ...
    def initializeByCopy(self,toCopy,iterationCount):
        ...
        # MN-LCS: Copy neural structure and apply Gaussian evolutionary weight mutation
        self.micro_mlp = copy.deepcopy(toCopy.micro_mlp)
        if hasattr(self.micro_mlp, 'coefs_'):
            for i in range(len(self.micro_mlp.coefs_)):
                noise = np.random.normal(0, 0.05, self.micro_mlp.coefs_[i].shape)
                self.micro_mlp.coefs_[i] += noise
