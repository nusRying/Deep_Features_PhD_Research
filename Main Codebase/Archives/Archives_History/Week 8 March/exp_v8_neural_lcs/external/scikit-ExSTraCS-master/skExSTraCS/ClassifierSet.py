...
class ClassifierSet:
    ...
    def updateSets(self,model):
        ...
        for ref in self.matchSet:
            ...
            # MN-LCS Core: Apply Negative Correlation Learning Penalty
            # Penalize redundancy to force specialized manifold learning
            if hasattr(self.popSet[ref], 'ncl_penalty'):
                # Cap the fitness degradation at 20% to prevent catastrophic collapse
                penalty_factor = max(0.8, 1.0 - (0.2 * self.popSet[ref].ncl_penalty))
                self.popSet[ref].fitness *= penalty_factor
