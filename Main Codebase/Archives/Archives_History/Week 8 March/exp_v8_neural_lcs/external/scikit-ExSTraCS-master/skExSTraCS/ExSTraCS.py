...
class ExSTraCS(BaseEstimator,ClassifierMixin):
    ...
    def runIteration(self,state_phenotype):
        ...
        if self.track_accuracy_while_fit:
            self.timer.startTimeEvaluation()
            prediction = Prediction(self,self.population, state_phenotype[0])
        ...
        # MN-LCS Core: Partial Fit Neural Networks
        # Teach the MLP local logic for the matched manifold
        all_classes = self.env.formatData.phenotypeList
        for ref in self.population.matchSet:
            cl = self.population.popSet[ref]
            try:
                # MLP learns to map the specific instance to the true class
                cl.micro_mlp.partial_fit([state_phenotype[0]], [state_phenotype[1]], classes=all_classes)
            except Exception:
                pass
