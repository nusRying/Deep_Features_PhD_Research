...
class Prediction():
    ...
    def getEvidentialProbabilities(self, K=1.0):
        ''' 
        Returns Dempster-Shafer Belief Masses: [prob_class_0, prob_class_1, ..., ignorance (Theta)]
        Uses a Consonant Belief Function approach where K represents the base prior ignorance.
        '''
        sorted_probs = []
        for k, v in sorted(self.vote.items()):
            sorted_probs.append(v)
            
        total_evidence = sum(sorted_probs)
        normalization_factor = total_evidence + K
        
        if total_evidence == 0:
            # Absolute ignorance
            masses = [0.0 for _ in range(len(sorted_probs))] + [1.0]
        else:
            # Belief masses for each class + Ignorance mass
            masses = [ev / normalization_factor for ev in sorted_probs] + [K / normalization_factor]
